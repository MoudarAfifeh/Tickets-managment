...
- Prisma ORM against PostgreSQL. Schema: `server/prisma/schema.prisma` (models: `User`, `Session`, `Ticket`, `Reply`, `Account`, `Verification`). Generated client output: `server/src/generated/prisma` (gitignored, regenerate with `bunx --bun prisma generate`).
- `Ticket` columns: `id, subject, body, bodyHtml?, status, category, senderName?, senderEmail, assignedToId?, createdAt, updatedAt`, mapped to table `ticket`. `status` (`TicketStatus`: open/resolved/closed) defaults to `open`; `category` (`TicketCategory`: general_question/technical_question/refund_request) defaults to `general_question`. `assignedToId` → `User` (`User.assignedTickets`, relation `AssignedTickets`).
- **Reply thread**: `Reply` (table `reply`) is one entry in a ticket's thread — `id, ticketId → Ticket (Ticket.replies, cascade delete), senderType, authorId? → User (User.replies), body, bodyHtml?, createdAt`. `bodyHtml` is the optional HTML rendering of `body` (mirrors `Ticket.bodyHtml`); nothing writes or renders it yet — `POST /api/tickets/:id/replies` stores plain `body` only. `senderType` (`SenderType`: agent/customer) tells the two sides apart: an `agent` reply has an `author` (the signed-in user who wrote it); a `customer` reply has `author: null` — its sender is the ticket's own `senderName`/`senderEmail`, not a separate field. Only the `agent` side has a producer right now (`POST /api/tickets/:id/replies`, below) — nothing creates `customer` replies yet, so the inbound webhook's "threaded" follow-up emails still aren't stored (see Webhooks below); that's the natural next extension. Outbound is also stored-only: no mail provider is wired, so an agent reply is never actually emailed to the customer, just persisted and shown in the thread.
  - `GET /api/tickets/:id` includes `replies` (oldest first, with `senderType` and `author: {id, name} | null`) via the shared `ticketDetailInclude` in `server/src/routes/tickets.ts`; `POST /api/tickets/:id/replies` (`requireAuth` only, body `{ body }` validated by `createReplySchema` in `code/src/schemas/tickets.ts`) always creates a `senderType: "agent"` reply authored by `req.user.id` and returns the full updated ticket. Frontend: see **Ticket detail page** below.
- **Ticket detail page** (`/tickets/:id`): `client/src/pages/TicketDetailsPage.tsx` is layout only — it owns the `["ticket", id]` query, the `BackLink` (`client/src/components/BackLink.tsx`, the reusable "← label" link to a parent page: `<BackLink to="/">Back to tickets</BackLink>`; use it for any future back navigation) and the loading / 404 / error states, and passes the **whole `ticket` object as a single `ticket` prop** to every section (type `TicketDetail`, plus `Reply`, in `client/src/types/ticket.ts` — the shape of `GET /api/tickets/:id`). The page owns the two `Card`s and their column spans; the sections render only `CardHeader`/`CardContent` pieces inside them:
  - Left card: `client/src/components/TicketDetails.tsx` (subject as the card title, ticket number, sender, date, message — no replies), `Replies.tsx` (the thread; renders nothing when empty; agent vs. customer replies distinguished by a `senderType` `Badge`, and by whose name shows: `author.name` for agent, `ticket.senderName`/`senderEmail` for customer), and `ReplyForm.tsx` (react-hook-form + `createReplySchema`; the reply box, invalidating `["ticket", ticket.id]` on success).
  - Right card: `UpdateTicket.tsx` — the status / category / assignee selects (it owns the assignees query and the three PATCH mutations, each writing the returned ticket into `["ticket", id]` so the page re-renders) plus the read-only sender and created date.
  - A new section of the page should follow the same shape: take `ticket: TicketDetail`, don't re-fetch it.
- Read via `GET /api/tickets` (`server/src/routes/tickets.ts`, `requireAuth` only — any signed-in user, not admin-gated), sorted newest first (`orderBy: { createdAt: "desc" }`). Frontend: `client/src/pages/Tickets.tsx` (TanStack Query + `TicketsTable`) is the `/` route — the ticket list is the app's home page.
...
- **Error text goes through `ErrorMessage`** (`client/src/components/ErrorMessage.tsx`) — a `<p>` styled `text-sm text-destructive`. Use it for every inline error (react-hook-form field errors, `serverError`, query `error.message`) instead of hand-writing `<p className="... text-destructive">`. It's a thin wrapper over `ComponentProps<"p">`, so pass a `className` to tweak it (merged with `cn`, so `className="text-xs"` overrides the default size and `className="mt-1"` adds spacing) — e.g. `<ErrorMessage className="text-xs">{errors.body.message}</ErrorMessage>` in `ReplyForm.tsx`. Default export. Not a shadcn primitive, so it lives in `components/`, not `components/ui/`.
...
- **Any schema needed by both the client and the server must be defined once in `code/` and imported from there — never duplicated per side.** Put it in its own file under `code/src/` (e.g. `code/src/user.ts`), export the schema plus its inferred type (`export type XInput = z.infer<typeof xSchema>`), and re-export both from `code/src/index.ts`. Consumers add `"code": "workspace:*"` to their `package.json` and `bun install` from the repo root to link it, then `import { xSchema, type XInput } from "code"`. See `code/src/user.ts` (`createUserSchema`/`editUserSchema`), used from `server/src/routes/users.ts` and `client/src/components/UserForm.tsx`.
  ...
  - Layout inside `code/src/`: flat schema files at the root (`user.ts`), enum mirrors under `constants/` (`ticket-status.ts`, `ticket-category.ts`), Zod schemas grouped under `schemas/` (`tickets.ts` → `inboundEmailSchema`/`InboundEmailInput`). Everything is re-exported from `code/src/index.ts`.
  - **A Prisma enum mirrored in `code/` is a plain union type, not a `const` object** — e.g. `export type TicketStatus = "open" | "resolved" | "closed";`, nothing more. No runtime object, no label maps — that belongs in whichever UI component needs it (see `TicketsTable.tsx`'s local `CATEGORY_LABELS`/`STATUS_VARIANT`). Consumers that need the actual runtime value use a plain string literal (`status: "open"`, not `TicketStatus.open`) — see `server/src/routes/webhooks.ts`.
...
- The one error handler is `server/src/middleware/errorHandler.ts`, registered last in `server/src/index.ts` (after all routes). It maps known errors to responses — currently Prisma `P2002` (unique constraint) → `409` and `P2025` (record not found) → `404` — and falls through to a logged `500`. Add new known-error mappings there, not in individual routes.
...
## Webhooks & email ingestion

- Inbound calls from external services live under `/api/webhooks/*` (`server/src/routes/webhooks.ts`, mounted in `index.ts`). No session — the caller is a machine. Each route is gated by `requireWebhookSecret` (`server/src/middleware/requireWebhookSecret.ts`): reads `WEBHOOK_SECRET` from env (**500** if unset), then compares it (constant-time) against the `x-webhook-secret` header **or** a `?secret=` query param (**401** if missing/wrong). `WEBHOOK_SECRET` is a **non-fatal** env var — `server/src/env.ts` only warns when it's absent (unlike the `required` list) — but the webhook is unusable until it's set. It's in `.env`, `.env.example`, `.env.test`, `.env.test.example`.
- `POST /api/webhooks/inbound-email` turns a support email into a `Ticket`. No mail provider (SendGrid/Mailgun) is wired — the body is a **provider-neutral JSON payload** validated by `inboundEmailSchema` (`code/src/schemas/tickets.ts`): `{ from (email, max 254), fromName? (max 255), subject (1–255), body (1–20,000), bodyHtml? (max 50,000) }` — the caps keep the payload under `express.json()`'s default 100kb limit. A provider adapter that flattens a real webhook into this shape is a later change.
- **Basic threading**: the subject is stripped of any leading `Re:`/`Fwd:`/`Fw:` run, then an existing **open** ticket with the same `senderEmail` and same subject (case-insensitive) folds the email in — responds `200 { ticket, threaded: true }` with the existing ticket, no mutation. Otherwise a new ticket is created (`status: open`, `category: general_question`) → `201 { ticket, threaded: false }`. A `Reply` model exists now (see Database above) and its `senderType: "customer"` variant is meant for exactly this, but nothing wires it up yet — a "thread" here is still just the match, and the follow-up email's content is not stored. Creating a `customer` `Reply` on the matched ticket instead of a no-op is the natural next change.
...
- Server side mirrors this with two composable middlewares in `server/src/middleware/`: `requireAuth` (401 if `req.user` is unset — populated per-request by the global `attachSession` middleware in `index.ts`) and `requireAdmin` (403 if `role !== "admin"`; it assumes `requireAuth` already ran and does its own auth check). An admin-only router applies both once via `router.use(requireAuth, requireAdmin)` rather than repeating them per route — see `server/src/routes/users.ts`, where every `/api/users` route is admin-only.
...
**Every new feature or behavior change needs test coverage before it's considered done** — a new route, page, or user-visible flow gets Playwright e2e coverage (delegate to `e2e-test-writer`, see below); a new/changed client component or page **always** gets a Vitest unit/component test alongside it per the conventions below, in the same change as the feature. This isn't situational — even when e2e coverage already exercises the flow end to end, still add the Vitest test; the two are complementary; e2e isn't a substitute. For server-only logic (a new route, validation, service function) with no Vitest harness yet, cover it manually (curl/`Bash`) as part of the change and note that in the PR description. Don't treat any of this as optional cleanup for later — add the test in the same change as the feature.
...
- Ticket-detail tests build their `ticket` with `makeTicket(overrides)` from `client/src/test/fixtures.ts` instead of re-declaring a base ticket per file.
