...
- Prisma ORM against PostgreSQL. Schema: `server/prisma/schema.prisma` (models: `User`, `Session`, `Ticket`, `Account`, `Verification`). Generated client output: `server/src/generated/prisma` (gitignored, regenerate with `bunx --bun prisma generate`).
- **`Ticket` is self-contained** — no `Message`/thread model. Columns: `id, subject, body, bodyHtml?, status, category, senderName?, senderEmail, assignedToId?, createdAt, updatedAt`, mapped to table `ticket`. `status` (`TicketStatus`: open/resolved/closed) defaults to `open`; `category` (`TicketCategory`: general_question/technical_question/refund_request) defaults to `general_question`. `assignedToId` → `User` (`User.assignedTickets`, relation `AssignedTickets`).
