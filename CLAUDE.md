...
- `POST /api/webhooks/inbound-email` turns a support email into a `Ticket`. No mail provider (SendGrid/Mailgun) is wired — the body is a **provider-neutral JSON payload** validated by `inboundEmailSchema` (`code/src/schemas/tickets.ts`): `{ from (email), fromName?, subject (min 1), body (min 1), bodyHtml? }`. A provider adapter that flattens a real webhook into this shape is a later change.
