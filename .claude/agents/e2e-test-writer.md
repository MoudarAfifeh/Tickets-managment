...
- `server/.env.test` is loaded automatically alongside `server/.env` whenever `NODE_ENV=test` (Bun's built-in `.env`/`.env.test` precedence). `server/package.json`'s `dev:test`, `db:test:migrate`, `db:test:seed` scripts all set `NODE_ENV=test` for this reason. **This is an overlay, not a full replacement**: `.env.test` values win, but a var that exists only in `.env` (not redeclared in `.env.test`) still leaks through into `NODE_ENV=test` processes. Don't assume a var is unset in tests just because `.env.test` doesn't mention it — check `.env` too, and if a secret/key genuinely needs to be absent or different in tests (e.g. `OPENAI_API_KEY`), it must be explicitly set (even to `""`) in `.env.test` to shadow it.
